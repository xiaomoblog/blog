...
vuex是一个状态管理工具，也是一个数据共享工具。它能够创建一个独立于组件之外的一个共享数据仓储，将需要的数据放在store当中，当需要用到数据的组件通过this.$store.state.xxx获取 
...
此种方式只能在小型项目中使用，当需要大量组件需要传值时，会通过其他不相干的组件层层传递，导致组件之间耦合严重，维护相关吃力
...
同步的修改state，mutations中不能使用异步操作，用setTimeoutt等异步操作会出错 ,需要放在action中
...
异步的修改state，它不能直接操作state，需要调用mucations中的方法来操作
...
用于对store中的数据进行加工形成新的数据，但不会影响store中的数据。store的数据变化之后，getter中的数据也会变化
...
对store进行模块划分。使用场景：当项目组中有多个人同时开发时，如果放在一个文件里面则会出现修改冲突，这里需要使用modules进行文件切分再组合。
...
只有使用ts的环境才能使用注解方式,通过调研有以下两种方案，其中[vue-property-decorator](https://github.com/kaorun343/vue-property-decorator)的星数遥遥领先(4.2K)。至于怎么选择，看实际项目的情况下，有的时候不一定有发言权，leader定哪个就用哪个吧。

-   [vue-property-decorator](https://github.com/kaorun343/vue-property-decorator)：非官方维护，一定学习成本
-   [vuex-class](https://github.com/ktsn/vuex-class)：非官方维护，在 vue-class-component 基础上补充一定`vuex`支持（支持有限）

我能自己做选择的话，我更倾向于使用[vue-property-decorator](https://github.com/kaorun343/vue-property-decorator)。毕竟流行程度高的话说明使用的人多，有坑的话查问题能找到的概率也大一些。
