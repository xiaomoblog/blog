...
### 五. 如何选择
`Carthage`我没有去研究过,附一篇别人的文章吧 。google出来多半都是使用`SPM`和`cocoapods`,所以也就主要研究了一下这两种。前面提到目前 `swift 3.0.1`只支持`mac os`和 `linux`系统,所以开发ios应用的话选用`cocoapods`会好一些,我会花了更多的时间在`cocoapods`上面。但是，我更期待`SPM`能够支持`IOS`,让我们拭目以待吧。
